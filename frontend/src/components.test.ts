...
describe('Dashboard', () => {
  ...
  it('offers a metric selector that controls dashboard cards', async () => {
    ...
    await flushPromises()
    selector.vm.$emit('update:modelValue', ['0:已用配额', '0:总配额'])
    selector.vm.$emit('change', ['0:已用配额', '0:总配额'])
    await flushPromises()
    expect(wrapper.findAll('.mcard')).toHaveLength(2)
    ...
    await flushPromises()
    expect(remounted.findComponent({ name: 'ElSelect' }).props('modelValue')).toEqual(['0:已用配额', '0:总配额'])
